...
function buildInner(){
  ...
  // Alpha: auto-resume the saved world, if any. A fresh start runs under N-body
  // gravity; a save keeps whatever it was saved with.
  if(!restoreSystemState() && !nbodyOn) nbEnable();
...
/* ---- ♻ Reset, 📂 Load and ⬆ Import in place ----
   A reload re-fetches, re-decodes and re-uploads every texture: many seconds
   on a slow connection, for a page that already has everything it needs. So
   the system is put back in the page instead: damage healed, custom worlds
   removed, deleted book worlds rebuilt, and every book world returned to the
   orbit, parent and phase this page opened it with. (Phases are random per page load, so "pristine" is the
